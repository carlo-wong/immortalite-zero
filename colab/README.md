...
Phase **2A** from iter **161** — same as `lightning-ai/run_train.py`. See `TRAINING_CHANGELOG.md`.
...
| `replay_buffer` / `replay_window` | **120k** | fresher replay (~7 iters at 128 games) |
| `draw_penalty` | 1/3 | football 3-1-0 shaping |
| `resign` | False | no self-play resignation |
| `lr` / `lr_min` | **5e-4 → 2e-4** | warm restart iters 161–196 |
| `lr_warmup_iters` | 161 | peak LR at resume iter |
| `lr_total_iters` | 196 | cosine decay end |
| `RESET_OPTIMIZER` | **False** (default) | set `True` for first iter-161 resume only |
...
Resume from `ckpt_iter_0160` with `RESET_OPTIMIZER = True` for the first launch only; leave `False` afterward.
